...
class sosat:
    ...
    def plot_beam(self,field_name = None, output_picture_name = 'co_cx_rot_beam.png',cmap = 'jet',plot = True):
        ...
        NN = np.where((np.abs(E_co)**2 + np.abs(E_cx)**2 )/peak > 10**(-20/10))[0]
        ...
        if plot:
            ...
            p1 = ax[0].pcolor(x,y, 10*np.log10(np.abs(E_co.reshape(Ny,Nx))**2),vmax = vmax,vmin= vmax-60,cmap = cmap)
            ax[0].axis('equal')
            ax[0].set_title('co-polar beam',fontsize = 15)
            p2 = ax[1].pcolor(x,y, 10*np.log10(np.abs(E_cx.reshape(Ny,Nx))**2),vmax = vmax,vmin= vmax-60,cmap = cmap)
            ...
            p3 = ax[0].pcolor(x,y, 10*np.log10(np.abs(E_co_new.reshape(Ny,Nx))**2),vmax = vmax,vmin= vmax-60,cmap = cmap)
            p4 = ax[1].pcolor(x,y, 10*np.log10(np.abs(E_cx_new.reshape(Ny,Nx))**2),vmax = vmax,vmin= vmax-60,cmap = cmap)
