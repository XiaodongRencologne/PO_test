...
'''
def PO(face1,face1_n,face1_dS,face2,Field_in_E,Field_in_H,k,parallel=True,device ='T.device('cuda)'):
    # output field:
    Field_E=vector();
    Field_H=vector();    
    ...
    def calculus1_GPU(x1,y1,z1,x2,y2,z2,N,ds,Je): 
        Field_E_x=np.zeros(x2.shape)+1j*np.zeros(x2.shape);
        Field_E_y=np.zeros(x2.shape)+1j*np.zeros(x2.shape);
        Field_E_z=np.zeros(x2.shape)+1j*np.zeros(x2.shape);
        Field_H_x=np.zeros(x2.shape)+1j*np.zeros(x2.shape);
    ...
    def calculus2(x1,y1,z1,x2,y2,z2,N,ds,Je,Jm):
        ...
        return Field_E_x,Field_E_y,Field_E_z,Field_H_x,Field_H_y,Field_H_z;
    if Jm_in==0:
        if parallel and (device == T.device('cpu'):
            Field_E.x,Field_E.y,Field_E.z,Field_H.x,Field_H.y,Field_H.z=calculus1(face1.x,face1.y,
                                                                                  face1.z,
                                                                                  face2.x,face2.y,face2.z,
                                                                                  face1_n.N,face1_dS,JE)
        elif parallel and (device == T.device('cuda'):
            Field_E.x,Field_E.y,Field_E.z,Field_H.x,Field_H.y,Field_H.z=calculus1_GPU(face1.x,face1.y,
                                                                                  face1.z,
                                                                                  face2.x,face2.y,face2.z,
                                                                                  face1_n.N,face1_dS,JE)
    else:
